...
class CDDataAugmentation:
    ...
    def transform(self, imgs, labels, to_tensor=True):
        ...
        # vertical flip (swap left and right)
        ...
        if self.with_random_crop and random.random() > 0:
            # size = self.img_size size after crop
            # img = imgs[0] size before crop (destination areas)
            # the top-left coordinates are denoted as (i, j)
